            ...
            for next_i, next_j, ops in [
                ...
                if 0 <= next_i and 0 <= next_j:
                    if self.dp[next_i][next_j] < self.dp[i][j]:
                        self.dfs(next_i, next_j, path + [[next_i, next_j, ops]])
                    elif (
                        self.dp[next_i][next_j] == self.dp[i][j]
                        and next_i == i - 1
                        and next_j == j - 1
                    ):
...
def filter_boxes_v2(query_box, query_text, ocr_boxes, ocr_texts, threshold=0.5):
    ...
    for ind in binds:
        ...
        if len(ocr_texts[ind]) == 0 or not is_worth(
            ocr_texts[ind], query_text, max_bind == ind
        ):
            continue
        mask[ind] = 1
    return mask
...
def hack_xy(text):
    ...
    if len(text) > 6 and text.startswith('~$'):
        xy = text.split('~')[1][1:].split('.')  # ~$x.y~xxx --》 x,y
...
def match_label_v1(gt_bbox, gt_text, ocr_bboxes, ocr_texts):  # 标签匹配逻辑
    """
...
if __name__ == "__main__":
    gt_bbox = [[[296, 142], [494, 142], [494, 210], [296, 210]]]
    gt_text = '123'
    ocr_bboxes = [[[296, 142], [494, 142], [494, 210], [296, 210]]]
    ocr_texts = ['212121123']
